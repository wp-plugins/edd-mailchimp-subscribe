...
function eddms_email_filter($fields){
    ...
    $fields[] = array(
                      ...
                      'css'       => 'min-width:300px;',
                      'type'      => 'select',
                      'options'   =>  call_user_func('eddms_get_mailchimp_lists', get_option('eddms_mailchimp_api_key')),
                      );
...
*/
function eddms_get_mailchimp_lists($apikey){
    $mailchimp_lists = unserialize(get_transient('eddms_mailchimp_mailinglist'));

    $settings = get_option('edd_settings');
    if(empty($mailchimp_lists) || $settings['eddms_force_refresh'] == '1'){
        require_once 'lib/edd-MCAPI.class.php';
